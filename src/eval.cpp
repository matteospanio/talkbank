...
void usage() {
	...
	puts("+c : create EVAL database file (eval -c +t*PAR +lengu) OR (eval -c +t*PAR)");
		...
		case 's':
			...
			if (*f == '[') {
				if (*(f+1) == '-') {
					...
					maingetflag(f-2,f1,i);
				} else if (uS.mStricmp(f, "[+ exc]") == 0){
					fprintf(stderr, "Please specify only pre-codes \"[- ...]\" or post-codes \"[+ ...]\"  with +/-s option.\n");
